...
public class AddToWishlist extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
        ...
        try {
            ...
            ResultSet rs1= st.executeQuery("Select * from wishlist where product_id = "+productid+" and username='"+u_name+"'");
            ResultSet rs=st1.executeQuery(q);
            if(rs.next()&& !rs1.next()) {
            	if(rs.getInt("availability")==0  ) {
		            String query = "insert into wishlist(product_id, username) values(?, ?)";
		            
		            PreparedStatement pStatement = conn.prepareStatement(query);
		            pStatement.setInt(1, productid);
		            pStatement.setString(2, u_name);
